...
export default function AuthCallback(){
  ...
  async function resolve(confirm:boolean){
    ...
    try{
      ...
      toast.success(confirm?'GitHub 已连接到当前账号，原账号已自动解绑':'已取消，原有绑定保持不变');
  ...
  return <AuthShell>
    ...
    <div className="auth-form-heading"><h1>{error?'授权尚未完成':transfer?'将 GitHub 连接到此账号？':'正在连接你的账号'}</h1><p>{error?'这次授权尚未完成，你可以重新选择登录方式。':transfer?'这个 GitHub 已连接到另一个 AtomForge 账号。':'马上就能回到你的创作空间。'}</p></div>
    {error?<><p role="alert" className="auth-callback-copy">{error}</p><div className="auth-callback-links"><Link to="/auth">返回登录 →</Link><Link to="/account">返回账号设置</Link></div></>:transfer?<>
      <div className="rounded-xl border bg-muted/30 p-4 space-y-3 text-sm leading-relaxed">
        <p><strong>GitHub @{transfer.login}</strong> 将连接到当前账号 <strong>{transfer.target_name}</strong>。</p>
        <p className="text-muted-foreground">确认后，原 AtomForge 账号将自动解绑，无法再通过这个 GitHub 登录或使用此连接发布仓库。原账号需通过其他登录方式访问。</p>
