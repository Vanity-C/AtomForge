...
export default function TeamBoard({run,mode='team'}:{run:StudioRun|null;mode?:'team'|'build'}) {
  ...
  return <section className="pipeline-board" aria-label={mode==='team'?'团队协作流水线':'工程师工作流水线'}>
    ...
    <div className="pipeline-detail" id="pipeline-stage-detail" key={selected.id}>
      ...
      {output&&(selected.id==='qa'?<QaReport output={output}/>:<details className="pipeline-output"><summary><span>实际交接文档</span><ChevronDown size={15}/></summary><div className="pipeline-output-body"><p>{output.summary||output.goal}</p>{!!(output.items||output.tasks)?.length&&<ul>{(output.items||output.tasks||[]).map((item,i)=><li key={i}>{item}</li>)}</ul>}{!!output.acceptance?.length&&<><h5>验收标准</h5><ul>{output.acceptance.map((item,i)=><li key={i}>{item}</li>)}</ul></>}{!!output.issues?.length&&<><h5>需要修复</h5><ul>{output.issues.map((item,i)=><li key={i}>{item}</li>)}</ul></>}{!!output.tests?.length&&<><h5>开发自测步骤</h5><ol>{output.tests.map((test,i)=><li key={i}><code>{test.action} {test.selector} {test.value}</code></li>)}</ol></>}</div></details>)}
    </div>
