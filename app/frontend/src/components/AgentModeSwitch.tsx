...
export function loadAgentMode(account = ''): AgentMode {
  if (!account) return 'build';
  try {return localStorage.getItem(`atomforge.agentMode:${account}`)==='team'?'team':'build';} catch {return 'build';}
}
export function saveAgentMode(mode:AgentMode, account = '') {
  if (!account) return;
  try {localStorage.setItem(`atomforge.agentMode:${account}`,mode);} catch {/* Selection works without browser storage. */}
}
...
export default function AgentModeSwitch({value,onChange,disabled=false,compact=false}:{value:AgentMode;compact?:boolean;onChange:(mode:AgentMode)=>void;disabled?:boolean}) {
  const engineer=useModeTeam('build').engineer?.name||'工程师';
