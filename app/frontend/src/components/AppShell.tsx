...
import { Atom, LayoutGrid, LogOut, Settings2, User } from 'lucide-react';
...
export function TopBar({ authState, user, center, right, brandTo = '/' }: TopBarProps) {
  ...
  return (
    <header className={`flex shrink-0 items-center gap-3 border-b border-border bg-card px-4 ${right ? 'min-h-14 flex-wrap py-2 sm:h-14 sm:flex-nowrap sm:py-0' : 'h-14'}`}>
      <BrandMark to={brandTo} />
      {center ? <div className="flex min-w-0 flex-1 items-center gap-2">{center}</div> : <div className="flex-1" />}
