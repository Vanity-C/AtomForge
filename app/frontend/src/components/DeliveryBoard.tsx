...
import {reportIssues} from '@/lib/qaIssues';
...
function StrategyEditor({run,board}:{run:StudioRun;board:DeliveryWorkflow}){
  const [policy,setPolicy]=useState<StrategyPolicy>({...board.policy,max_test_steps:board.policy.max_test_steps??1000});
  ...
  return <form className="delivery-strategy-form" onSubmit={save}>
    <div className="delivery-strategy-fields">
      ...
      {([{key:'wip',label:'进行中 WIP 上限',max:3,min:1},{key:'sla_minutes',label:'工作包 SLA（分钟）',max:1440,min:1},{key:'max_repairs',label:'自动修复上限',max:30,min:0},{key:'min_tests',label:'独立测试最少步骤',max:Math.min(16,policy.max_test_steps??1000),min:2},{key:'max_test_steps',label:'测试步骤上限',max:10000,min:policy.min_tests}] as const).map(field=><div key={field.key}><Label htmlFor={'strategy-'+field.key}>{field.label}</Label><Input id={'strategy-'+field.key} type="number" min={field.min} max={field.max} required disabled={terminal||busy} value={policy[field.key]} onChange={e=>setPolicy({...policy,[field.key]:Number(e.target.value)})}/></div>)}
    </div>
    <p className="delivery-help">当前执行器每次执行一个专业工作包；提高 WIP 不会自动增加模型并发。成员快照本轮固定，更换团队在下次任务生效。SLA 超时标记提醒，不自动跳过门禁。</p>
    <p className="delivery-help">测试步骤上限默认 1000，可设为 2–10000，且不能低于独立测试最少步骤。全部独立场景共享上限，不必凑满。调整从下一轮计划或恢复验收时生效，执行中的计划保持原上限。</p>
...
export default function DeliveryBoard({run}:{run:StudioRun}){
  ...
  const [view,setView]=useState<'delivery'|'strategy'|'qa'>('delivery');
  ...
  const issueCount=qa?reportIssues(qa).length+(qa.verification?.pendingIssues?.length??0):0;
  return <section className="delivery-board" aria-label="团队双层看板">
    ...
    <div className="delivery-tabs flex-wrap" role="group" aria-label="选择看板"><button aria-pressed={view==='delivery'} onClick={()=>setView('delivery')}><Layers3 size={16}/>交付看板</button><button aria-pressed={view==='qa'} onClick={()=>setView('qa')}><ShieldCheck size={16}/>测试问题{qa?` · ${issueCount}`:''}</button><button aria-pressed={view==='strategy'} onClick={()=>setView('strategy')}><Gauge size={16}/>领导战略看板</button></div>
    {view==='qa'?<QaPanel key={run.id} run={run}/>:view==='strategy'?<div className="delivery-strategy"><h4>管理策略，不逐站审批</h4><p>领导处理目标、优先级、容量、等待与异常升级。日常交接由专业负责人直接完成；独立测试不可关闭。</p><StrategyEditor key={run.id+':'+board.revision} run={run} board={board}/><details><summary>策略变更记录 · {board.policy_history.length}</summary>{board.policy_history.map(entry=><p key={entry.revision}>r{entry.revision} · {entry.reason}</p>)}</details></div>:<>
      ...
      <p className="delivery-help">{board.policy.priority==='urgent'?'加急':'标准'}泳道 · 进行中 WIP {board.cards.filter(c=>c.state==='doing').length}/{board.policy.wip} · 阻塞留在原列 · 完成本轮检查后集中反馈</p>
      ...
      {qa&&<div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border p-3"><div><strong className="text-sm">测试问题 · {issueCount} 项</strong><p className="text-xs text-muted-foreground">{qa.verified?'本轮验收已通过':'查看问题类型、严重等级、描述及验证依据'}</p></div><Button size="sm" variant="outline" onClick={()=>setView('qa')}>查看问题详情</Button></div>}
