...
export default function MembersPanel({projectId}:{projectId:number}){
  ...
  return <div className="border-t pt-4"><h3 className="font-semibold">项目成员</h3><p className="my-2 text-xs text-muted-foreground">通过已注册邮箱添加成员，无需发送邮件。编辑者可修改代码和生成应用；发布和云服务配置由所有者管理。</p>
    ...
    {data?.can_manage&&<div className="space-y-2"><Input placeholder="成员的注册邮箱" value={email} onChange={e=>setEmail(e.target.value)}/><select className="w-full rounded border bg-background p-2 text-sm" value={role} onChange={e=>setRole(e.target.value)}><option value="viewer">查看者</option><option value="editor">编辑者</option></select><Button variant="outline" disabled={!email.trim()} onClick={()=>void mutate('POST')}>添加 / 更新成员</Button></div>}
  </div>;
}
