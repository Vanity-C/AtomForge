...
def test_strategy_changes_are_versioned_private_and_do_not_reset_delivery(client,monkeypatch):
    ...
    assert board['policy']['max_test_steps']==1000
    request={**board['policy'],'priority':'urgent','sla_minutes':20,'max_repairs':30,'max_test_steps':1500,'revision':board['revision'],'reason':'集中处理已确认的阻塞'}
    ...
    assert fresh['result']['workflow']['policy']['max_test_steps']==1500
    ...
    for extra in ({'wip':0},{'min_tests':0},{'max_repairs':31},{'max_test_steps':10001},{'max_test_steps':True},{'max_test_steps':2,'min_tests':4},{'states':['anything']},{'reason':''}):
