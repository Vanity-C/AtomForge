...
class Scenario(BaseModel):
    ...
    tests: list[dict] = Field(min_length=2, max_length=MAX_REVIEW_STEPS)
...
class Review(BaseModel):
    ...
    issues: list[str] = Field(max_length=48)
    tests: list[dict] = Field(default_factory=list, max_length=MAX_REVIEW_STEPS)
...
def validate_review(raw, minimum=2):
    review = Review.model_validate(raw).model_dump()
    review['issues'] = unique_issues(review['issues'])
    ...
    if scenarios:
        ...
        for scenario in scenarios:
            steps.extend(validate_tests(scenario['tests']))
    ...
    validate_tests(review['tests'], minimum=minimum)
    return review
...
优先返回 scenarios，格式为 [{"name":"独立场景名称","tests":[测试步骤]}]，通常按独立业务能力划分 2–6 个场景，简单应用可只用 1 个。每个场景从全新隔离浏览器及空存储开始，必须自带前置操作并以断言结束，不得依赖别的场景创建的数据。每个场景至少 2 步，所有场景合计最多 48 步，不为凑数量重复测试。使用 scenarios 时不另写 tests；旧版单场景 tests 格式仍兼容。单个场景失败会停止该场景的依赖步骤，其他独立场景继续执行，最终合并反馈。
