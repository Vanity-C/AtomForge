...
class User_settingsService:
    """Service layer for User_settings operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[User_settings]:
        """Create a new user_settings"""
        try:
            if user_id:
                data['user_id'] = user_id
            obj = User_settings(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created user_settings with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating user_settings: {str(e)}")
            raise
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for user_settings {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[User_settings]:
        """Get user_settings by ID (user can only see their own records)"""
        try:
            query = select(User_settings).where(User_settings.id == obj_id)
            if user_id:
                query = query.where(User_settings.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching user_settings {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of user_settingss (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(User_settings.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(User_settings, field):
                        column = getattr(User_settings, field)
            ...
            stmt = select(User_settings, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(User_settings, field_name):
                        stmt = stmt.order_by(getattr(User_settings, field_name).desc())
                else:
                    if hasattr(User_settings, sort):
                        stmt = stmt.order_by(getattr(User_settings, sort))
            else:
                stmt = stmt.order_by(User_settings.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(User_settings)
                for condition in conditions:
            ...
            logger.error(f"Error fetching user_settings list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[User_settings]:
        """Update user_settings (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"User_settings {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated user_settings {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating user_settings {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete user_settings (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"User_settings {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted user_settings {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting user_settings {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[User_settings]:
        """Get user_settings by any field"""
        try:
            if not hasattr(User_settings, field_name):
                raise ValueError(f"Field {field_name} does not exist on User_settings")
            column = getattr(User_settings, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(User_settings).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching user_settings by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[User_settings]:
        """Get list of user_settingss filtered by field"""
        try:
            if not hasattr(User_settings, field_name):
                raise ValueError(f"Field {field_name} does not exist on User_settings")
            column = getattr(User_settings, field_name)
            ...
            logger.error(f"Error fetching user_settingss by {field_name}: {str(e)}")
            raise