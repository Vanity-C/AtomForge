...
# ------------------ Service Layer ------------------
class Af_usersService:
    """Service layer for Af_users operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Af_users]:
        ...
        try:
            ...
            logger.info(f"Created af_users with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating af_users: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Af_users]:
        """Get af_users by ID"""
        try:
            query = select(Af_users).where(Af_users.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching af_users {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of af_userss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Af_users, field):
                        column = getattr(Af_users, field)
            ...
            stmt = select(Af_users, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Af_users, field_name):
                        stmt = stmt.order_by(getattr(Af_users, field_name).desc())
                else:
                    if hasattr(Af_users, sort):
                        stmt = stmt.order_by(getattr(Af_users, sort))
            else:
                stmt = stmt.order_by(Af_users.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Af_users)
                for condition in conditions:
            ...
            logger.error(f"Error fetching af_users list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Af_users]:
        """Update af_users"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Af_users {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated af_users {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating af_users {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Af_users {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted af_users {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting af_users {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Af_users]:
        """Get af_users by any field"""
        try:
            if not hasattr(Af_users, field_name):
                raise ValueError(f"Field {field_name} does not exist on Af_users")
            column = getattr(Af_users, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Af_users).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching af_users by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Af_users]:
        """Get list of af_userss filtered by field"""
        try:
            if not hasattr(Af_users, field_name):
                raise ValueError(f"Field {field_name} does not exist on Af_users")
            column = getattr(Af_users, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Af_users.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching af_userss by {field_name}: {str(e)}")
            raise