...
app = FastAPI(
    title="AtomForge API",
    description="DeepSeek-driven app generation and project workspace",
    version="1.0.0",
    lifespan=lifespan,
)
...
from middlewares.demo_limits import DemoLimits
app.add_middleware(DemoLimits)
...
def frontend_file(path: str):
    ...
    if not target.is_relative_to(FRONTEND_DIST) or not target.is_file():
