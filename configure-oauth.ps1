...
function Show-Status {
    ...
    foreach ($provider in @('github', 'gitee')) {
        $prefix = 'ATOMFORGE_' + $provider.ToUpper()
        $configured = (Get-Setting ($prefix + '_CLIENT_ID')) -and (Get-Setting ($prefix + '_CLIENT_SECRET'))
        Write-Host ($provider + ': ' + $(if ($configured) { 'credentials present (not yet verified with provider)' } else { 'application credentials missing' }))
        Write-Host ('Callback: ' + $origin + '/api/v1/af-auth/oauth/' + $provider + '/callback')
    }
}
...
foreach ($provider in @('github', 'gitee')) {
    Write-Host ('Callback for ' + $provider + ': ' + $taskOrigin + '/api/v1/af-auth/oauth/' + $provider + '/callback')
    $taskId = Read-Host ($provider + ' OAuth Client ID [Enter skips this provider]')
    if (-not $taskId.Trim()) { continue }
    if ($taskId.Trim() -notmatch '^[A-Za-z0-9_.-]+$') { throw 'Invalid Client ID format.' }
    $taskSecure = Read-Host ($provider + ' OAuth Client Secret (hidden)') -AsSecureString
    ...
    try {
        ...
        $taskPrefix = 'ATOMFORGE_' + $provider.ToUpper()
...
Show-Status
Write-Host 'Saved locally. Restart the AtomForge backend, then authorize each provider in the browser to verify sign-in.'
